...
export function TokenChart({ address, symbol, currentPrice, initialTrades = [] }: { address: string; symbol: string; currentPrice?: number | null; initialTrades?: AnalystTrade[] }) {
  ...
  return (
    <section className="card min-w-0 overflow-hidden p-4 md:p-6" aria-label={`${symbol} chart`}>
      ...
      {view === "price" && <div className="mt-3 border-t border-border pt-3"><p className="text-xs text-secondary">Trade timeline · {markerTrades.length} recorded swaps · <span className="text-neon">Buy</span> / <span className="text-negative">Sell</span></p>{timeline.length ? <div className="relative mx-4 my-3 h-20 border-b border-border" aria-label="Trade timeline">{timeline.map(g => <button key={g.id} title={`${g.trade.trader.name} ${g.trade.side}`} aria-label={`Timeline ${g.trade.side} ${g.trade.trader.name}, ${g.trades.length} trades`} onClick={() => setSelected(g.trades)} className={`absolute flex h-8 min-w-8 -translate-x-1/2 items-center justify-center rounded-full border bg-background text-[10px] ${g.trade.side === "BUY" ? "top-0 border-neon text-neon" : "top-10 border-negative text-negative"}`} style={{ left: `${(g.t - timelineStart) / Math.max(1, timelineEnd - timelineStart) * 100}%` }}><TraderAvatar name={g.trade.trader.name} id={g.trade.trader.id} avatar={g.trade.trader.avatar} size="xs" /><span>{g.trades.length > 1 ? g.trades.length : g.trade.side === "BUY" ? "B" : "S"}</span></button>)}</div> : <p className="py-3 text-xs text-muted">No recorded swaps match these timeline filters.</p>}<p className="text-[11px] text-muted">Timeline shows timestamps, not prices. Price-chart markers require a known USD execution price and a USD chart. Nearby trades are grouped; select one for details. Up to 200 stored swaps plus incoming trades.</p></div>}
