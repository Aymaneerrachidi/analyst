...
export function TokenChart({ address, symbol, currentPrice, initialTrades = [] }: { address: string; symbol: string; currentPrice?: number | null; initialTrades?: AnalystTrade[] }) {
  ...
  return (
    <section className="card min-w-0 overflow-hidden p-4 md:p-6" aria-label={`${symbol} chart`}>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4 text-[11px] leading-relaxed text-muted"><a href="https://www.tradingview.com/" target="_blank" rel="noreferrer">TradingView Lightweight Charts{String.fromCharCode(8482)} {String.fromCharCode(183)} {String.fromCharCode(169)} 2025 TradingView, Inc.</a>
