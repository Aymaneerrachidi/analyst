...
export function LiveLeaderboard({ initial, initialStatus, period, filter, query, page }: { initial: AnalystTrader[]; initialStatus: { wallets: number; automatic: boolean } | null; period: RankingPeriod; filter: string; query: string; page: number }) {
  const params = new URLSearchParams({ period, filter, q: query, limit: '25', offset: String((page - 1) * 25) });
  const { data, isError } = useQuery({ queryKey: ['leaderboard', period, filter, query, page], queryFn: ({ signal }) => apiGet<{ traders: AnalystTrader[]; rankingStatus?: { wallets: number; automatic: boolean } | null }>(`/api/traders?${params}`, signal), initialData: { traders: initial, rankingStatus: initialStatus }, refetchInterval: 30_000, staleTime: 15_000 });
