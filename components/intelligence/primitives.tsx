...
export function IntelligenceNav() {
  return <nav aria-label="Intelligence tools" className="flex flex-wrap gap-2 py-4">{[['/radar', 'Runner Radar'], ['/signals', 'Signal history'], ['/narratives', 'Narratives'], ['/compare', 'Compare'], ['/feed', 'Your feed'], ['/positions', 'Positions']].map(([href, label]) => <Link key={href} href={href} className="rounded-full border border-border px-3 py-2 text-xs text-secondary hover:border-neon hover:text-neon">{label}</Link>)}</nav>;
}
