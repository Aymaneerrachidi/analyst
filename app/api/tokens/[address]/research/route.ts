...
import { getResearch, generateResearch, reserveResearch, researchConfigured } from '@/lib/services/token-research';
import { guardWrite } from '@/lib/social/write-guard';
export const maxDuration = 180;
type Context = { params: Promise<{address:string}> };
export async function GET(_req:Request,ctx:Context) { const {address}=await ctx.params; if (!/^0x[a-f0-9]{40}$/i.test(address)) return Response.json({error:'Invalid token'},{status:400}); return Response.json({configured:researchConfigured(),state:await getResearch(address.toLowerCase())},{headers:{'Cache-Control':'no-store'}}); }
