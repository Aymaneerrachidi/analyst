...
- Read requests do not trigger ingestion on Vercel. QStash owns sync. Token metadata enrichment uses Next.js `after` so unfinished enrichment remains attached to the request lifecycle, within its duration limit.
...
QStash schedules `analyst-trades` every two minutes, and `analyst-full` at minutes 1, 16, 31 and 46. Each sends POST to `/api/internal/sync?kind=trades|full` with the secret Authorization header. Nominal usage is 816 deliveries daily, below QStash Free's 1,000/day allowance; retries also count. Monitor failures and quotas. These are periodic updates, not a continuous chain subscription. The UI continues to label data older than 60 seconds DELAYED.
