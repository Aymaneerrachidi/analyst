...
export async function webhookFreshness(): Promise<Freshness> {
  let provider: Freshness['provider'] = process.env.LIVE_FEED_SOURCE === 'stalkchain' ? 'stalkchain' : 'alchemy';
  ...
  const [independent] = await db.select().from(schema.appMeta).where(eq(schema.appMeta.key, 'public-feed:worker'));
  const independentState = independent?.value as { at?: string; status?: string } | undefined;
  const useIndependent = independentState?.at && Date.now() - Date.parse(independentState.at) < 45_000 && independentState.status === 'live';
  if (useIndependent) provider = 'chain';
  const state = (useIndependent ? independentState : row?.value) as { at?: string; status?: string; active?: boolean } | undefined;
  ...
  const live = ageMs !== null && ageMs < 30_000 && state?.status === 'live' && (provider === 'stalkchain' || provider === 'chain' || state.active === true);
