...
export type ResearchState = { status: 'pending' | 'ready' | 'unavailable'; at: string; report?: ResearchReport; message?: string };
...
async function save(token: string, value: ResearchState) { const db = await getDb(); await db.insert(schema.appMeta).values({ key: `research:token:${token}`, value }).onConflictDoUpdate({ target: schema.appMeta.key, set: { value, updatedAt: new Date() } }); }
export async function reserveResearch(token: string) {
  ...
  if (previous && Date.now()-Date.parse(previous.at)<(previous.status==='pending'?180_000:900_000) && previous.status !== 'unavailable') return { accepted: false, message: 'Using the existing report.', state: previous };
  ...
  const lock = await db.insert(schema.appMeta).values({ key: 'research:global-lock', value: { token } }).onConflictDoUpdate({ target: schema.appMeta.key, set: { value: { token }, updatedAt: new Date() }, setWhere: sql`${schema.appMeta.updatedAt} < now() - interval '3 minutes'` }).returning({ key: schema.appMeta.key });
